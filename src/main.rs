...
const USER_AGENT: &str = "https://github.com/DragonOS-Community/team";
...
fn main() {
    ...
    env.filter_module("dragonos_team", log::LevelFilter::Info);
    if std::env::var("DRAGONOS_TEAM_FORCE_COLORS").is_ok() {
...
fn run() -> Result<(), Error> {
    ...
    match cli {
        ...
        Cli::AddPerson { ref github_name } => {
            ...
            println!("Adding person: {}, id: {}", github_name, github_id);
        ...
        Cli::AddRepo { org, name } => {
            ...
            let repo = RepoToAdd {
                ...
                description: &repo.description.unwrap_or_else(|| {
                    format!("The {name} repo maintained by the DragonOS project")
                }),
        ...
        Cli::DumpWebsite => {
            println!(
                "# Autogenerated by `cargo run dump-website` in https://github.com/DragonOS-Community/teams_data"
            );
        ...
        Cli::EncryptEmail => {
            ...
            println!(
                "{}",
                dragonos_team_data::email_encryption::encrypt(&key, &plain)?
            );
        }
        Cli::DecryptEmail => {
            ...
            println!(
                "{}",
                dragonos_team_data::email_encryption::try_decrypt(&key, &encrypted)?
            );
        }
    }
